...
void gen_syscall_list(Cfg *cfg) {
  ...
  for(auto & ptr : ptrs) {
	...
	getFnInfo(df, cfg);
        ...
        if(ptr.second->addressTaken() && cfg->withinPltSec(ptr.first) == false)
...
void data_obj_points_to(map <uint64_t, DataObj *> &data_objs, Cfg *cfg) {
  ...
  while(dit != data_objs.end()) {
    ...
    while(it != pointer_storage.end() && it->first < cur_end) {
	  ...
	  if(objs.size() <= 0) {
	    dit->second->points_to.insert(data_seg_pointers.begin(), data_seg_pointers.end()); // Could not find an object corresponding to the data pointer. so add all AT.
	    break;
	  }
          for(auto & o : objs) {
...
unordered_set<uint64_t> addrs_points_to(uint64_t addrs, Cfg *cfg, map<uint64_t, DataObj *> &data_objs) {
  ...
  if(obj_list.size() <= 0) {
    res.insert(data_seg_pointers.begin(), data_seg_pointers.end());
    return res;
  }
...
unordered_set <uint64_t> compute_at_list(FuncInfo *info, Cfg *cfg, map <uint64_t, DataObj *> &data_objs) {
  ...
  for (auto & addr : all_referred_addrs) {
    if(fn_syscall_map.find(addr) != fn_syscall_map.end()) {
      cout<<"Adding referred AT function: "<<hex<<addr<<endl;
      at_set.insert(addr);
    }
    /*
    else {

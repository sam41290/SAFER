...
{
  _dl_printf_sbi("dl_open_worker called\n");
  ...
  for (unsigned int i = 0; i < new->l_searchlist.r_nlist; ++i)
    if (new->l_searchlist.r_list[i]->l_real->l_versions == NULL)
      (void) _dl_check_map_versions (new->l_searchlist.r_list[i]->l_real,0, 0);
...
{
  _dl_printf_sbi("dl_open called: %s\n",file);
  //if(program_instrumented()) {
  //  char *basefile = strrchr(file,'/');
  //  if(basefile != NULL) {
  ...
  if (__glibc_unlikely (exception.errstring != NULL))
  {
   ...
   state if relocation failed, for example.  */
    ...
    /* Release the lock.  */
    _dl_printf_sbi("Calling __rtld_lock_unlock_recursiven");
    __rtld_lock_unlock_recursive (GL(dl_load_lock));

	  ...
	  for (idx = 0; idx < size; ++idx)
	    if (entries[idx].name != NULL)
	      enter_unique_sym (newentries, newsize, entries[idx].hashval,
                                ...
                                entries[idx].map);
	//   tab->free = (void *)gtf((unsigned long)tab->free);
	  tab->free (entries);
	  tab->size = newsize;
	  size = newsize;
...
{
  _dl_printf_sbi("looking up symbol: %s\n", undef_name);
...
{
  if(map->l_name)
    _dl_printf_sbi("_dl_setup_hash called for: %s\n",map->l_name);
  Elf_Symndx *hash;
  ...
  if (__glibc_likely (map->l_info[ELF_MACHINE_GNU_HASH_ADDRIDX] != NULL))
    {
      ...
      _dl_printf_sbi("hash addr: %lx\n",(unsigned long)hash32);
      map->l_nbuckets = *hash32++;
      Elf32_Word symbias = *hash32++;
      Elf32_Word bitmask_nwords = *hash32++;
      _dl_printf_sbi("bitmask_nwords: %lx\n",(unsigned long)bitmask_nwords);

...
call_destructors (void *closure)
{
  ...
  _dl_printf_sbi("calling destructors: %s\n",map->l_name);

  if (map->l_info[DT_FINI_ARRAY] != NULL)
    {
      ElfW(Addr) *array = (ElfW(Addr) *) (map->l_addr
			              + map->l_info[DT_FINI_ARRAY]->d_un.d_ptr);
      unsigned int sz = (map->l_info[DT_FINI_ARRAYSZ]->d_un.d_val / sizeof (ElfW(Addr)));

      while (sz-- > 0) {
        unsigned long translated_ptr = gtf(array[sz]);
...
{
  _dl_printf_sbi("dl_close_worker called: %s\n",map->l_name);
  /* One less direct use.  */
...
_dl_close (void *_map)
{
  ...
  _dl_printf_sbi("_dl_close called: %s\n",map->l_name);

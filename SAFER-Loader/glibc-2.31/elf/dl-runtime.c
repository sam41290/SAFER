	   ...
	   struct link_map *l, ElfW(Word) reloc_arg)
{
  ...
  if (sym != NULL
    ...
    value = DL_FIXUP_VALUE_ADDR (value);
    _dl_printf_sbi("Calling gtf from sym look up: %lx\n",(unsigned long)value);
    value = gtf(value);
    value = elf_ifunc_invoke (DL_FIXUP_VALUE_ADDR (value));
    _dl_printf_sbi("look up returned irelative pointer: %lx\n", value);
  }
  else {
	/* Added code for SBI-Loader.  First check the symbol type.  */
	if (ELFW(ST_TYPE) (sym->st_info) == STT_FUNC) {
		_dl_printf_sbi("The objname is %s, ptr is 0x%lx and mapping start 0x%lx\n",
                result->l_name, value, result->l_addr);
        value = encode((unsigned long)value, (unsigned long)result->l_addr);
	}
    else {
      _dl_printf_sbi("look up returned non function type address: %lx\n", value);
    }
  }
	  ...
	  if ((l->l_audit_any_plt | result->l_audit_any_plt) != 0)
	      ...
	      for (unsigned int cnt = 0; cnt < GLRO(dl_naudit); ++cnt)
		{
		  ...
		  if ((l_state->bindflags & LA_FLG_BINDFROM) != 0
			...
			{
			// afct->symbind = (void *)gtf((unsigned long)afct->symbind);
			  uintptr_t new_value
  ...
  if (GLRO(dl_naudit) > 0
      ...
      for (unsigned int cnt = 0; cnt < GLRO(dl_naudit); ++cnt)
	{
	  if (afct->ARCH_LA_PLTENTER != NULL
		...
		// afct->ARCH_LA_PLTENTER = (void *)gtf((unsigned long)afct->ARCH_LA_PLTENTER );
	      uintptr_t new_value
		  ...
		  const void *inregs, void *outregs)
{
  ...
  for (unsigned int cnt = 0; cnt < GLRO(dl_naudit); ++cnt)
	...
	// afct->ARCH_LA_PLTEXIT = (void *)gtf((unsigned long)afct->ARCH_LA_PLTEXIT);

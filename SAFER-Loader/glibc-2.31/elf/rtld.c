...
//
//bool
//program_instrumented(void) {
//  return instrumented;
//}

struct link_map *inst_lib_map = NULL;
...
void
__segv_handler (int nSignum, siginfo_t * si, void *vcontext)
{
  ucontext_t *context = (ucontext_t *) vcontext;
  _dl_printf ("Segfault at RIP: %lx faulting pointer: %lx\n",
      (unsigned long)context->uc_mcontext.gregs[REG_RIP],(unsigned long)context->uc_mcontext.gregs[REG_CR2]);
  unsigned long fault_ptr = (unsigned long)context->uc_mcontext.gregs[REG_CR2];
  struct gtt_sbi *tmp = get_gtt_module(fault_ptr);
  if(tmp == NULL) {
    _dl_printf("Segfault from uninstrumented module..\n");
  }
  else if(is_encoded_ptr(fault_ptr)) {
    _dl_printf ("module: %s encoded ptr caused fault..perhaps a data ptr: %lx\n",tmp->name,fault_ptr);
  }
  else {
    if(is_old_ptr(fault_ptr,tmp)) {
      _dl_printf ("module: %s old ptr caused fault: %lx\n",tmp->name,fault_ptr);
    }
    else
      _dl_printf ("module: %s new ptr caused fault: %lx\n",tmp->name,fault_ptr);
  }
  unsigned long rip = (unsigned long)context->uc_mcontext.gregs[REG_RIP];
  struct gtt_sbi *calling_module = get_gtt_module(rip);
  if(calling_module!= NULL)
    _dl_printf("Calling module: %s\n",calling_module->name);
  _exit(-1);
}
...
_dl_start (void *arg)
{
  _dl_printf_sbi("_dl_start called\n");
#ifdef DONT_USE_BOOTSTRAP_MAP
...
dlmopen_doit (void *a)
{
  ...
  _dl_printf_sbi("dlmpoen_doit called\n");
...
do_preload (const char *fname, struct link_map *main_map, const char *where)
{
  ...
  if(strcmp(fname, "libinstrument.so") == 0) {
    inst_lib_map = args.map;
  }
...
load_audit_module (const char *name, struct audit_ifaces **last_audit)
{
 ...
 /* A null symbol indicates that something is very wrong with the
  ...
  // laversion = (void *)gtf((unsigned long)laversion);
	 ...
	 ElfW(auxv_t) *auxv)
{
  ...
  /* We save the the loader start and end map address in our own global variables.  */
    loader_map_start = GL(dl_rtld_map).l_map_start;
    loader_map_end = GL(dl_rtld_map).l_map_end;
    loader_l_addr = GL(dl_rtld_map).l_addr;

    vdso_start = (ElfW(Addr)) GLRO(dl_sysinfo_dso);
    const ElfW(Phdr) * vdso_ph = (const ElfW(Phdr) *)((const void *) GLRO(dl_sysinfo_dso) + GLRO(dl_sysinfo_dso)->e_phoff);
    int ph_num = GLRO(dl_sysinfo_dso)->e_phnum;
    unsigned long vdso_laddr = 0;
    for(int i = 0; i < ph_num; i++) {
      if(vdso_ph[i].p_type == PT_LOAD) {
        vdso_end = vdso_ph[i].p_vaddr + vdso_ph[i].p_memsz;
        if(vdso_laddr == 0)
          vdso_laddr = vdso_ph[i].p_vaddr;
      }
    }
    vdso_laddr = vdso_start - vdso_laddr;
    vdso_end = vdso_laddr + vdso_end;
    ...
    switch (ph->p_type)
    {
      ...
      case PT_NOTE:
	    if (_rtld_process_pt_note (main_map, ph))
	      _dl_error_printf ("ERROR: '%s': cannot process note segment.\n", _dl_argv[0]);
	    break;
      }

  //if(instrumented) 
  {
    //_dl_printf("Instrumented module: %s\n",main_map->l_name);
    fill_gtt(main_map); //Fill up the address translation table and create an entry in gtt
    /* set LD_LIBRARY_PATH to instrumened libraries directory*/
    GLRO(dl_inhibit_cache) = 1;
    const char *inst_libs = INST_DIRS;
    size_t path_size = 0;
    if(library_path != NULL && library_path[0] != '\0') {
      //_dl_printf("Pre env path: %s\n",library_path);
      path_size += strlen(library_path);
    }
    path_size += strlen(inst_libs) + 1;
    char *new_lib_path = (char *)malloc(path_size);
    int i;
    for(i = 0; library_path != NULL && library_path[i] != '\0'; i++)
        new_lib_path[i] = library_path[i];
    new_lib_path[i] = ':';
    strcpy(new_lib_path + i + 1,inst_libs);
    library_path = new_lib_path;

    //struct sigaction act;
    //act.sa_sigaction = __segv_handler;
    //act.sa_flags = SA_SIGINFO;
    //__sigaction (11, &act, NULL);
  }
  ...
  if(instrumented) {
	...
	tmp->name = GLRO(dl_sysinfo_map)->l_name;
	tmp->l_addr = GLRO(dl_sysinfo_map)->l_addr;
	tmp->start_addr = GLRO(dl_sysinfo_map)->l_map_start;
	tmp->end_addr = GLRO(dl_sysinfo_map)->l_map_end;
	tmp->next = NULL;
    struct att_rec *vdso_att = (struct att_rec *)malloc(sizeof(struct att_rec) * 10);
    vdso_att[0].old_ = (unsigned long)GLRO(dl_vdso_clock_gettime64);
    vdso_att[0].new_ = (unsigned long)GLRO(dl_vdso_clock_gettime64);
    vdso_att[1].old_ = (unsigned long)GLRO(dl_vdso_gettimeofday);
    vdso_att[1].new_ = (unsigned long)GLRO(dl_vdso_gettimeofday);
    vdso_att[2].old_ = (unsigned long)GLRO(dl_vdso_time);
    vdso_att[2].new_ = (unsigned long)GLRO(dl_vdso_time);
    vdso_att[3].old_ = (unsigned long)GLRO(dl_vdso_getcpu);
    vdso_att[3].new_ = (unsigned long)GLRO(dl_vdso_getcpu);
    vdso_att[4].old_ = (unsigned long)GLRO(dl_vdso_clock_getres_time64);
    vdso_att[4].new_ = (unsigned long)GLRO(dl_vdso_clock_getres_time64);
    //vdso_att[4].old_ = (unsigned long)GLRO(dl_vdso_clock_getres);
    //vdso_att[4].new_ = (unsigned long)GLRO(dl_vdso_clock_getres);
    tmp->att_start = (unsigned long)vdso_att;
    tmp->att_end = (unsigned long)vdso_att + sizeof(struct att_rec) * 5;
    setup_vdso_hash(tmp);
  }
  ...
  //if(instrumented) {
  //  /* set LD_LIBRARY_PATH to instrumened libraries directory*/
  //  char *inst_libs = getenv("INST_LIBS");
  //  if(inst_libs != NULL) {
  //      _dl_init_paths (inst_libs);
  //  }
  //  else{
  //    _dl_printf("env variable INST_LIBS not set\n");
  //    _exit(-1);
  //  }
  //}
  ...
  //PRELOAD instrumentation library
  {
    RTLD_TIMING_VAR (start);
    rtld_timer_start (&start);
    ...
    //(void) _dl_catch_error (&objname, &err_str, &malloced, map_doit, &args);
    //if (GL(dl_ns)[LM_ID_BASE]._ns_nloaded != old_nloaded)
    //  npreloads += 1;

    //inst_lib_map = args.map;

    npreloads += handle_preload_list ("libinstrument.so", main_map, "--preload");
    rtld_timer_accum (&load_time, start);
  }
  ...
  //LOAD CGOT SECTION FOR ALL INSTRUMENTED MODULES (SAFER)
  {
    struct link_map *l;
    for(l = main_map; l; l = l->l_next) {
      //if(strcmp(l->l_name,"libinstrument.so") == 0)
      //  inst_lib_map = l;
      fill_cgot(l);
    }
	struct relocate_args rl_args = { .l = inst_lib_map,
	  			.reloc_mode = ((GLRO(dl_lazy)
	  				       ? RTLD_LAZY : 0)
	  				       | __RTLD_NOIFUNC) };
	_dl_receive_error (print_unresolved, relocate_doit, &rl_args);
  }

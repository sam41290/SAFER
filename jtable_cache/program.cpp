...
const string& bin_path): faulty(false),
               ...
               if (b_curr->faulty) {
                  LOG4("error: missing fall-through target " << target);
                     ...
                     if (i->call()) {
                        ...
                        for (IMM i=0; i<(IMM)(ARCH::raw_bytes_hlt.length()); i+=3)
                           raw_bytes.push_back((uint8_t)Util::to_int("0x" +
                                               ARCH::raw_bytes_hlt.substr(i,2)));
                        i->replace(new Exit(Exit::EXIT_TYPE::HALT), raw_bytes);
...
vector<IMM> Program::scan_fptrs_in_gap() {
   ...
   for (auto it = sorted_insns_.begin(); it != sorted_insns_.end(); ++it) {
      if ((*it)->gap && (prev == nullptr || !prev->gap)) {
         for (; it != sorted_insns_.end() &&
         ((((*it)->offset() & 15) == 0) ||
         (*it)->to_string().compare("nop") == 0); ++it);

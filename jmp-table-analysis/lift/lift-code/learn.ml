...
(*******************************************************************************
One major change is the idenitifcation of parameters by the assembler parser.
Based on this, we will 
   (a) avoid branching on parameter positions, and 
   (b) limit transformation functions to operate on parameter positions
More explanation to be provided after completing the code.
*******************************************************************************)

(* TODO:
   1. Fix argpos and projMcp
   2. Experiment with and imlement merging
   3. check compatibility check
 *)
...
type sym = 
  | SCONST of string
  | ICONST of nativeint (* so that we can use all 64 bits *)

type paramTxFn =
  | EQV of sym
  | EQ of position
  | ADD of position * nativeint
  | MULT of position * nativeint 
  | DIV of position * nativeint
...
let isAsmLabel (s: string) : bool = 
    try ((String.sub s 0 4) = ".L0x") with _ -> false
...
let getValRels (pos:position) (vv:sym*sym) = 
  let getIntRel pos (i:nativeint) (o:nativeint) = 
    let addrel = 
...
(* We should probably prefer EQ, then DIV/MULT, then ADD. It seems coincidental
   matches for MULT and DIV are less likely than ADD, so in the absence of
   additional info, MULT/DIV are more likely to be correct than ADD. *)
...
let ordMfun (x:paramTxFn) (y:paramTxFn): int =
  ...
  | MULT(px, ix) -> 
     (match y with EQ(_) -> (-1)
     | MULT(py, iy) -> (compare (px,(NI.to_int ix)) (py,(NI.to_int iy))) 
     | _ -> 1)
  | DIV(px,ix) -> 
     (match y with EQ(_) -> (-1)
     | MULT(py, iy) -> (-1)
     | DIV(py,iy) -> (compare (px,(NI.to_int ix)) (py,(NI.to_int iy)))
     | _ -> (-1))
  | ADD(px,ix) -> 
     (match y with EQ(_) -> (-1)
     | MULT(py, iy) -> (-1)
     | DIV(py, iy) -> (-1)
     | ADD(py, iy) -> (compare (px,(NI.to_int ix)) (py,(NI.to_int iy)))
     | _ -> 1)
  | EQV(tx) -> 
     (match y with EQV(ty) -> (compare tx ty) 
     | _ -> (-1))

(* If the match function list mfl contains an EQ(v) then return v, else undef  *)
...
let string_of_paramTxFn (f:paramTxFn) = 
  let string_of_sifun op p i = 
    (NI.to_string i)^(op^"@")^(string_of_pos p)
  in
  match f with
...
(* Return the subterm of t at position p *)
...
(* Return the root symbol of term t *)
let rec getRoot (t:term): sym = match t with
  ...
  | AND(mfl)  -> 
     ...
     in getSymMfl mfl
(* Unhandled cases shoud raise exceptions, and that is correct *)
...
(* Remove from fr all the positions in the list pl *)
let removePosFr (pl:position list) (fr: fringe): fringe =
  List.filter (fun (p, t) -> not (List.mem p pl)) fr

let removeParamPosFr (fr: fringe): fringe =
  (List.filter 
    (fun (p, t) -> match t with PARAM(_) -> false | _ -> true) fr)    

(* Remove from all fringes in frl all the positions in the list pl *)
let removePosFrl (pl:position list) (frl: fringe list): fringe list =
  (List.map (removePosFr pl) frl)

let removeParamPosFrl (frl: fringe list): fringe list =
  (List.map removeParamPosFr frl)

(* Remove descendant positions of p in fr *)
let removeDescPosFr (p:position) (fr: fringe): fringe =
  let isAncestor p1 p2 = 
    let res = posComp p1 p2 in res = CEQ || res = CPRFX in
  let retainable (p1, t) = not (isAncestor p p1)
  in List.filter retainable fr

let getOutput (fr: fringe): fringe = removeDescPosFr [1] fr
let getOutputFrl (frl: fringe list) = List.map getOutput frl

let getInput  (fr: fringe): fringe = removeDescPosFr [2] fr
let getInputFrl (frl: fringe list) = List.map getInput frl

(*******************************************************************************
   Operations on fringe
*)
...
(* If p1 is a prefix of p2, return suffix of p2 immediately after p1, else [] *)
...
(* Return the term at position p in the fringe fr *)
...
(* Remaining cases left undefined on purpose --- considered an error *)

(* Remove all subterms of the output term from the fringe fr *)
let removeOutPos fr =
...
(* Remove all subterms of the input term from the fringe fr *)
let removeInPos fr =
...
(* Apply above pruning to all match functions appearing within a term *)
...
(* Apply above pruning to all match functions appearing within a fringe *)
let pruneRelations fr = List.map (fun (p, t) -> (p, (pruneRels t))) fr
...
let rec residue_term (p:position) (trm1:term) (trm2:term) ignNonOp: fringe = 
  if trm1 = ANY 
    then [] 
  else if trm1 = NONE 
    then [(p, NONE)] 
        ...
        | [] -> (
            ...
            | AND(mfl1) -> 
               if ((mcp_term false trm1 trm2) = trm2)
                  then [(p, trm1)]
               else (errmsg ("Unexpected case in residue_op "^(string_of_pos p) ^ " " ^ (string_of_term trm1) ^ " " ^ (string_of_term trm2)); [(p, trm1)])
        )
    ...
    | AND(mfl2) ->
       let AND(mfl1) = trm1 in
       let mfln = mcp_mfl mfl1 mfl2 in
       if (mfln <> []) then [] else [(p, NONE)]
    ...
    let rec reshelper fr1 fr2 = match fr2 with
      ...
      | (p2, trm2)::fr2l -> match fr1 with
        ...
        | (p1, trm1)::fr1l -> 
           ...
           | CEQ -> (List.append (residue_term p1 trm1 trm2 false) 
                       (reshelper fr1l fr2l))
           | _ -> errmsg ("mcp_frl called on p1=" ^ (string_of_pos p1) ^
                          " and p2=" ^ (string_of_pos p2));
                raise Exit;
  ...
  let substMf mf = match mf with
    ...
    | ADD(p, i) -> 
      if isIntVal (getSymFr p inp) then
        let ICONST(iv) = getSymFr p inp in
        (OP(ICONST(NI.add iv i),[]))
      else (errmsg ("Unexpected fringe " ^ string_of_fringe inp ^ " in substMf");
            raise Exit) 
    | MULT(p, i)-> 
      if isIntVal (getSymFr p inp) then
        let ICONST(iv) = getSymFr p inp in
        (OP(ICONST(NI.mul iv i),[]))
      else (errmsg ("Unexpected fringe " ^ string_of_fringe inp ^ " in substMf");
            raise Exit) 
    | DIV(p, i) -> 
      if isIntVal (getSymFr p inp) then
        let ICONST(iv) = getSymFr p inp in 
        (OP(ICONST(NI.div iv i),[]))
      else (errmsg ("Unexpected fringe " ^ string_of_fringe inp ^ " in substMf");
            raise Exit) 
  in
...
let subst (t:term) (out:fringe) (inp:fringe): term =
  ...
  let helper t1 (p, otr) = substPos t1 p otr false
  in
  (List.fold_left helper t nfr)  

let substFr (fr: fringe) (subfr: fringe) =
  let helper t1 (p, otr) = substPos t1 p otr false in
  let substTerm t = (List.fold_left helper t subfr)  
  in (List.map (fun (p, t) -> (p, (substTerm t))) fr)

module SymSet =
  Set.Make(struct let compare = Pervasives.compare type t = sym end)
...
let stateCompare (st1:state) (st2:state): int = 
  ...
  | BINST(i1, p1, v1, brck1, e11, e12) -> (
    ...
    | BINST(i2, p2, v2, brck2, e21, e22) ->
       Pervasives.compare (p1, v1, brck1, e11, e12) (p2, v2, brck2, e21, e22)
    | NWAYST _ -> -1
  )
  | NWAYST(i1, p1, vel1) -> (
    match st2 with
    | BINST _ -> 1                    
    | NWAYST(i2, p2, vel2) -> Pervasives.compare (p1,vel1) (p2,vel2)
  )
...
let rec elimMfs (t:term) (p:position) pos_set : term =
  ...
  | AND(mfl) -> AND([]) (* Will this case ever arise? *)
  | _ -> t
...
let edgeCompare (e1:edge) (e2:edge): int =
  ...
  | TERM(eid11, mcp1) -> (
     ...
     | TERM(eid2, mcp2) -> 
        ...
        in Pervasives.compare pr_mcp1 pr_mcp2
     | NORM _ -> -1
  )
  | NORM(i1, mcp1, st1) -> (
    match e2 with
    ...
    | NORM(i2, mcp2, st2) -> 
       let pr_mcp1 = (projMcp mcp1) and
           pr_mcp2 = (projMcp mcp2) 
       in Pervasives.compare (st1, pr_mcp1) (st2, pr_mcp2)
  )
...
let rec mkState (rsduel: fringe list) (snum:stateId) p branch: stateId*stateId =
 ...
 (* Depending on the return value of split, construct a two-way or multi-way
  ...
  | NWAY(val_rsdues) ->
     ...
     let sval_eid_ls = List.sort Pervasives.compare val_eid_ls in
...
and
    mkEdge (rsduel:fringe list) (snum:stateId): stateId*edgeId =
      ...
      let (pos, branch, conf) = 
        try
          if final then ([], (minval, 1), 1.0) else select rsdueln
        with _ -> ([], (minval, 1), 0.0)
      in
      ...
      let (final, mcp, rsdueln) = 
        if (conf < 1.0) then
            ...
            let rsdueln1 = (List.filter (fun f -> (f <> [([], NONE)])) (residue_frl merged rsduel))
            in 
               if (isFinal rsdueln1)
                  then (true, merged, rsdueln1)
               else (final, mcp, rsdueln)
        else (final, mcp, rsdueln)
      in
      if final || pos = [] then
        ...
        let rsduelnn = removeParamPosFrl rsdueln in (
          (* At this point, the only unconsumed input should be parameters.
             ...
             output at all. While this is *possible* in rare cases, it is more
             likely to be an error *)
          if final && List.exists (fun x -> x <> []) rsduelnn then
            errmsg ("Ignored incompatible input: "^(string_of_frlist rsduelnn))
          else ()
          );
        let eid = snum in
        let nedge = TERM(eid, mcp) in
        (
...
let mkducer asmRtlRules: stateId =
  try (
    ...
    let _ = msg 1 ("**** Automaton completed with " ^ (string_of_int nstates)
                   ^ " states ****")
    in sid (* that is the stateId of the start state *)
  ) with _ -> let _ = Printexc.print_backtrace stderr in 1
...
let dot_of_auto (outf: out_channel) (a: stateId): unit = 
  ...
  let prtEdge (src:int) (dst:int) (eid:int) (lbl:string) 
    ...
    let pr1 fr = (Str.global_replace (Str.regexp ", ") 
                    "\\n" (string_of_fringe fr))
    in 
    output_string outf (pr1 (*pruneRelations*) mcp);
...
(*******************************************************************************)
(* Below is the code for lifting (translating) input to output. Lifting uses   *)
(* the automaton, and is relatively simple, but is currently complicated by    *)
(* compatibility check code. But I am skeptical about the placement as well as *)
(* the need for this compatibility check here.                                 *)

let assertCompat (inp:fringe) eid ((p:position), (t2:term)): bool =
  let t1 = getposFr p inp in
  let x = residue_term p t2 t1 true in
  match x with
      (_,NONE)::_ ->
        raise (Incompatible_Input(t1, t2, p, eid))
    | _ -> false
(*
  with Incompatible_Input -> 
    (errmsg ("Incompatible input "^(string_of_fringe inp)^ " found." ^
                " It cannot be matched against "^(string_of_term t2)^" at "^
                (string_of_pos p) ^ " T1: " ^ (string_of_term t1) ^ " T2: " ^
                (string_of_term t2));
     raise Incompatible_Input)
 *)

let assertCompatFr (inp:fringe) (outp:fringe) eid: bool =
  List.mem true (List.map (assertCompat inp eid) outp)
...
and 
  ...
  | TERM(n, fr) ->
     ...
     let _ = assertCompatFr inp frin eid in

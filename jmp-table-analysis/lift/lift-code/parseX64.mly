...
%token <string> ID
%token <string> PREFIX
%token DOLLAR PERCENT STAR COLON POUND LPAREN RPAREN /* X86 */
...
| PREFIX ident args SEMICOLON { 
      Learn.OP(SCONST($1),
               [Learn.OP(SCONST($2^"_"^(string_of_int(List.length $3))), $3)])
  }
| PREFIX insn SEMICOLON {
      Learn.OP(SCONST($1), [$2])
}
;
...
| reg { $1 }
| PERCENT ident LPAREN INT RPAREN /* special cases: replace st(1) by st1 */
  {
    let num = $4 in
...
| PERCENT ident COLON arg /* [fg]s:arg */
  { Learn.OP(SCONST($2), [$4]) }
| INT LPAREN nonmem_args RPAREN    /* X86 mem ref syntax */
  {
    let l = if $1 = NI.zero 
...
nonmem_arg:
| INT                              { Learn.OP(SCONST("addr"), [Learn.PARAM(ICONST($1))]); }
| DOLLAR INT                       { Learn.OP(SCONST("imm"), [Learn.PARAM(ICONST($2))]); }

...
(******************************************************************************
                                  C Interface
******************************************************************************)
let prepare_filename (session:int) (id:int): string =
    "/tmp/sbr2/" ^ string_of_int(session) ^ "/tmp_" ^ string_of_int(id)
;;

let c_train1_imap (sessionId:int) (trainFile:int) =
  let f = prepare_filename sessionId trainFile in
  let asmRtlPairs = (readallpairs f "") in
  (Learn.mkducer (procRules asmRtlPairs))
;;

let c_train2_imap (sessionId:int) (trainFile1:int) (trainFile2:int) =
  let f1 = prepare_filename sessionId trainFile1 in
  let f2 = prepare_filename sessionId trainFile2 in
  let asmRtlPairs = (readallpairs f1 f2) in
  (Learn.mkducer (procRules asmRtlPairs))
;;

let c_load_automaton (sessionId:int) (autoFile:int) =
  let f = prepare_filename sessionId autoFile in
  (Learn.load_automata (open_in_bin f))
;;

let c_lift_asm (sessionId:int) (asmFile:int) (rtlFile:int) =
  let f1 = prepare_filename sessionId asmFile in
  let f2 = prepare_filename sessionId rtlFile in
  (doLiftAsm [f1] f2)
;;

      ...
      if (!dest_id.bad()) {                                             \
         ...
         if (bin != nullptr) {                                          \
            ...
            if (bin->op() == BinaryOP::OP::AND) {                         \
               ...
               cstr.intersect(AbsCstr(dest_id,                          \
                        Range(0, c1 != _oo && c1 > 0? c1:               \
                                (c0 != _oo && c0 > 0? c0: oo))));       \
            }                                                           \
            else if (bin->op() == BinaryOP::OP::LSHIFTRT) {               \
               auto c1 = bin->operand_const(1);                         \
               if (c1 != _oo) {                                         \
                  IMM x = (8*(IMM)(source->mode_size()) - c1);          \
                  x = ((IMM)1 << x) - 1;                                \
                  cstr.intersect(AbsCstr(dest_id, Range(0,x)));         \
               }                                                        \
            }                                                           \
         }                                                              \
         /* 0 <= ebx == extend(al) <= 256 */                            \
            ...
            destination->mode_size() > 1) {                             \
               ...
               cstr.assign(dest_id, src_id);                            \
               cstr.intersect(AbsCstr(dest_id, r & Range(0,255)));      \
            }                                                           \
            else                                                        \
               cstr.assign(dest_id, src_id);                            \
         }                                                              \

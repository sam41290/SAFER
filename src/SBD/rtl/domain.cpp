...
void BaseStride::unit_mem(const BaseStride& object, uint8_t width) {
   if (object.t == T::DYNAMIC || object.t == T::TOP) {
      //cout<<"[unit_mem] Dynamic/Top "<<object.b<<" "<<(int)object.s<<endl;
      unit_type(object.t);
      #if ENABLE_SUPPORT_CONSTRAINT
         range = Range::FULL;
      #endif
   ...
   //if (object.t == T::MEM) {
      ...
      w = width;
      x = object.unit_clone();
      //cout<<"[unit_mem] Mem "<<b<<" "<<(int)s<<endl;
   }
   //else if (object.t == T::DYNAMIC || object.t == T::TOP) {
   //   t = T::MEM;
   //   b = 0;
   //   s = object.s;
   //   w = width;
   //   x = (object.x == nullptr)? nullptr: object.x->clone();
   //   cout<<"[unit_mem] Dynamic/Top "<<b<<" "<<(int)s<<endl;
   //}
      ...
      w = width;
      x = (object.x == nullptr)? nullptr: object.x->clone();
      //cout<<"[unit_mem] NMem/CONST "<<(int)b<<" "<<(int)s<<endl;
...
void BaseStride::add(const BaseStride& object) {
   if (t == T::BOT)
	 ...
	 //cout<<"[add] X "<<(int)X->t<<" stride "<<(int)X->s<<" Y "<<(int)Y->t<<" stride "<<(int)Y->s<<endl;
...
void BaseStride::mul(const BaseStride& object) {
   if (t == T::BOT) {
      //cout<<"[mul] This object is BOT"<<endl;
      return;
   }
   else if (object.t == T::BOT) {
      //cout<<"[mul] Incoming object is BOT"<<endl;
      type(T::BOT);
   }
   else {
      ...
      for (const BaseStride* Y = &object; Y != nullptr; Y = Y->next) {
         ...
         if ((X->t == T::NMEM || X->t == T::CONST || X->t == T::DYNAMIC) && X->s == 0) {
            auto c = X->b;
            if (Y->t == T::DYNAMIC || Y->t == T::TOP || Y->t == T::MEM) {
	       //cout<<"[mul] X Nmem/Const "<<c<<" Y Dynamic"<<endl;
               Z->t = T::NMEM;
               Z->b = 0;
               Z->s = c;
               Z->w = 0;
	       ...
	       //cout<<"[mul] X Nmem/Const "<<c<<" Y NMem/Const "<<Y->b<<" "<<(int)Y->s<<endl;
               Z->t = T::NMEM;
               Z->b = Y->b * c;
               Z->s = Y->s * c;
               Z->w = 0;
         ...
         else if ((Y->t == T::NMEM || Y->t == T::CONST || Y->t == T::DYNAMIC) && Y->s == 0) {
            auto c = Y->b;
            if (X->t == T::DYNAMIC || X->t == T::TOP || X->t == T::MEM) {
	       //cout<<"[mul] Y Nmem/Const "<<c<<" X Dynamic"<<endl;
               Z->t = T::NMEM;
               Z->b = 0;
               Z->s = c;
               Z->w = 0;
	       ...
	       //cout<<"[mul] Y Nmem/Const "<<c<<" X NMem/Const "<<(int)X->b<<" "<<X->s<<endl;
               Z->t = T::NMEM;
               Z->b = X->b * c;
               Z->s = X->s * c;
               Z->w = 0;

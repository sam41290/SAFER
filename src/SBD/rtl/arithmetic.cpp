...
vector<string> Unary::readsReg() {
   vector<string> res;
   IF_RTL_TYPE(Reg, operand_, reg, {
      if (reg->reg() != ARCH::flags)
         res.push_back(X86_64::to_string(reg->reg()));
   }, { res = operand_->readsReg(); });
   return res;
}
// ----------------------------------- BinaryOP ----------------------------------
BinaryOP::~BinaryOP() {
...
string BinaryOP::to_string() const {
   if (op_ == OP::ANY)
      return string("");
   return string("(").append(BinaryOP::OP_STR[(int)op_])
                     .append(mode_string()).append(" ")
...
bool BinaryOP::equal(RTL_EQUAL eq, RTL* _v) const {
   if (_v == nullptr)
      return (eq == RTL_EQUAL::PARTIAL);
...
vector<RTL*> BinaryOP::find(RTL_EQUAL eq, RTL* _v) {
   vector<RTL*> vList;
   if (equal(eq, _v))
      vList.push_back(this);
...
vector<string> BinaryOP::readsReg() {
   vector<string> res;
   for (int i = 0; i < 1; i++) {
      IF_RTL_TYPE(Reg, operand(i), reg, {
         if (reg->reg() != ARCH::flags)
            res.push_back(X86_64::to_string(reg->reg()));
      }, { res = operand(i)->readsReg(); });
   }
   return res;
}

Expr* BinaryOP::clone() {
   return new BinaryOP(op_, expr_mode(),
                     operands_[0]->clone(), operands_[1]->clone());
}


AbsVal BinaryOP::eval(State& s) {
   EVAL_BINARY(s);
}


#if ENABLE_SUPPORT_CONSTRAINT == true
   const AbsId& BinaryOP::expr_id(const State& s) {
      if (!run_expr_id_)
         return expr_id_;
      run_expr_id_ = false;
   ...
   const AbsPair& BinaryOP::expr_pair(const State& s) {
      if (!run_expr_pair_)
         return expr_pair_;
...
bool BinaryOP::contains(RTL* rtl) const {
...
vector<string> Compare::readsReg() {
   return expr_->readsReg();
}

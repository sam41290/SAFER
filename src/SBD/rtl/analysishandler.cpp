...
/* -------------------------------- AnalysisHandler --------------------------------- */
AnalysisHandler::AnalysisHandler(const vector<tuple<IMM,RTL*,vector<uint8_t>>>& offset_rtl_raw,
const vector<IMM>& fptr_list, const unordered_map<IMM,unordered_set<IMM>>& icfs,
bool total_cfg): faulty(false),
...
void AnalysisHandler::build_func(IMM entry, const unordered_map<IMM,unordered_set<IMM>>& icfs,
...
/* -------------------------------------------------------------------------- */
void AnalysisHandler::block_split(Insn* insn) {
   /* [           insn      ] */
   /* [         ][          ] */
   /*      b        b_next    */
   auto b = insn->parent;
...
void AnalysisHandler::block_connect(Block* b, IMM target, COMPARE cond, bool fix_prefix) {
...
void AnalysisHandler::block_dfs(Insn* i) {
...
/* -------------------------------------------------------------------------- */
SBAFunction* AnalysisHandler::func(IMM fptr) {
   //std::cout<<"[SBA] Looking for the function entry: "<<std::hex<<fptr<<std::endl;
   checked_fptrs_.insert(fptr);
   auto it_f = f_map_.find(fptr);
   if (it_f != f_map_.end())
      return it_f->second;
   else {
      //std::cout<<"[SBA] Could not find the function entry: "<<std::hex<<fptr<<std::endl;
      auto it_b = b_map_.find(fptr);
      if(it_b == b_map_.end()) {
          //std::cout<<"[SBA] Could not find basic block for entry: "<<std::hex<<fptr<<std::endl;
	  return NULL;
      }
      //++Framework::num_func;
      auto b = it_b->second;
      //std::cout<<"[SBA] Creating new function function entry: "<<std::hex<<fptr<<std::endl;
      auto f = new SBAFunction(this, b);
      if (f->faulty) {
...
void AnalysisHandler::fptrs(const vector<IMM>& fptr_list) {
...
#if ENABLE_DETECT_UPDATED_FUNCTION
   void AnalysisHandler::propagate_update(Block* b) {
...
bool AnalysisHandler::updated(IMM fptr) {
   #if ENABLE_DETECT_UPDATED_FUNCTION
...
void AnalysisHandler::update() {
   ...
   for (auto offset: recent_fptrs_) {
      std::cout<<"[SBA] Building func: "<<std::hex<<offset<<std::endl;
...
/* -------------------------------------------------------------------------- */
void AnalysisHandler::icf(IMM jump_loc, const unordered_set<IMM>& targets) {
   if (targets.empty()) {
      icfs_[jump_loc] = {};
      return;
   }
...
//
//   bool AnalysisHandler::valid_icf(IMM target, SBAFunction* func) const {
//      if (valid_icf(target)) {
...
//
//
//   void AnalysisHandler::resolve_unbounded_icf() {
...
#endif
/* -------------------------------------------------------------------------- */
//void AnalysisHandler::load_binary() {
//   BINARY::load_binary(bin_path_,info_);
//}
//
//
//uint64_t AnalysisHandler::read_value(int64_t offset, uint8_t width) const {
//   return BINARY::read_value(info_, offset, width);
//}
//
//
//unordered_set<IMM> AnalysisHandler::definite_fptrs() const {
//   return BINARY::definite_fptrs(info_, bin_path_);
//}
//
//
//unordered_set<IMM> AnalysisHandler::prolog_fptrs() const {
...
//
//
//unordered_set<IMM> AnalysisHandler::scan_cptrs() const {
//   /* stored cptrs */
...
//
//
//vector<IMM> AnalysisHandler::scan_fptrs_in_gap() {
//   Insn* prev = nullptr;

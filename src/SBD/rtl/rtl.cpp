...
RTL::operator BinaryOP*() const {
   auto t = (Arithmetic*)(*this);
   if (t == nullptr) return nullptr;
   return t->arith_type()==Arithmetic::ARITH_TYPE::BINARY ?
          (BinaryOP*)this : nullptr;
}
...
vector<string> Parallel::readsReg() {
   //cout<<"Regs read - parallel"<<endl;
   vector<string> res;
   for (auto stmt: stmts_) {
      auto r = stmt->readsReg();
      res.insert(res.end(), r.begin(), r.end());
   }
   return res;
}

vector<string> Parallel::writesReg() {
   vector<string> res;
   for (auto stmt: stmts_) {
      auto r = stmt->writesReg();
      res.insert(res.end(), r.begin(), r.end());
   }
   return res;
}
...
vector<string> Sequence::readsReg() {
   //cout<<"Regs read - Sequence"<<endl;
   vector<string> res;
   for (auto stmt: stmts_) {
      auto r = stmt->readsReg();
      res.insert(res.end(), r.begin(), r.end());
...
vector<string> Sequence::writesReg() {
   vector<string> res;
   for (auto stmt: stmts_) {
      auto r = stmt->writesReg();
      res.insert(res.end(), r.begin(), r.end());
...
vector<string> Assign::readsReg() {
   //cout<<"Regs read - Assign"<<endl;
   vector<string> res = src_->readsReg();
   IF_RTL_TYPE(Reg, dst_, reg, {},{
      auto dst_reads = dst_->readsReg();
      res.insert(res.end(), dst_reads.begin(), dst_reads.end());
   });
   return res;
}
vector<string> Assign::writesReg() {
   vector<string> res;
   IF_RTL_TYPE(Reg, dst_, reg, {
      if (reg->reg() != ARCH::flags)
         res.push_back(X86_64::to_string(reg->reg()));
   }, {});
   return res;
}
   ...
   void Assign::assign_flags(const State& s) {
      IF_RTL_TYPE(Reg, dst()->simplify(), reg, {
         if (reg->reg() == ARCH::flags) {
            ...
            auto bin = (BinaryOP*)(*src()->simplify());
...
vector<string> Call::readsReg() { 
   return target_->readsReg(); 
}

vector<string> Call::writesReg() { 
   vector<string> res; 
   return res; 
}
...
vector<string> Clobber::readsReg() {
   vector<string> res;
   return res;
}
vector<string> Clobber::writesReg() {
   vector<string> res;
   IF_RTL_TYPE(Reg, expr_, reg, {
      if (reg->reg() != ARCH::flags)
         res.push_back(X86_64::to_string(reg->reg()));
...
vector<string> Exit::readsReg() {
   vector<string> res;
   return res;
}
vector<string> Exit::writesReg() {
   vector<string> res;
   return res;
}
...
vector<string> Nop::readsReg() {
   vector<string> res;
   return res;
}
vector<string> Nop::writesReg() {
   vector<string> res;
   return res;
}

...
IMM SBAFunction::offset() const {
   return entry_->offset();
}


void SBAFunction::summary() {
...
void SBAFunction::tarjan(Block* u, IMM& cnt, stack<Block*>& st) {
   ++cnt;
   u->num = cnt;
   u->low = cnt;
   st.push(u);
...
void SBAFunction::rev_postorder(Block* header) {
...
void SBAFunction::analyse(const State::StateConfig& conf) {
   LOG3("############# analyzing ##############");
...
vector<AbsVal> SBAFunction::track(TRACK trackType, const UnitId& id,
...
vector<ExprLoc> SBAFunction::find_def(ARCH::REG reg, const Loc& loc) const {
   vector<ExprLoc> res;
...
vector<RetType> SBAFunction::find_pattern(const ExprLoc& X,
...
#if ENABLE_RESOLVE_ICF
void print_jtable(IMM loc, BaseStride* expr, SBAFunction* func) {

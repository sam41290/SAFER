...
IMM stackSym = get_sym(ARCH::stack_ptr);
IMM staticSym = get_sym(ARCH::insn_ptr);
/* -------------------------------------------------------------------------- */
              /*   0   initReg[]   initStack[]   initStatic[]   */
IMM get_sym(REGION r, IMM i) {
   switch (r) {
...
IMM get_sym(ARCH::REG r) {
   return get_sym(REGION::REGISTER,(IMM)r);
}


IMM get_sym(const UnitId& id) {
   return get_sym(id.r(),id.i());
}


UnitId get_id(REGION r, IMM i) {
...
UnitId get_id(ARCH::REG r) {
   return get_id(REGION::REGISTER,(IMM)r);
}


UnitId get_id(IMM sym) {
   if (sym == oo)
